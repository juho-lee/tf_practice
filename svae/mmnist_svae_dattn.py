...
z_w_mean = linear(hid_t_enc, 1, scope='z_w_mean')
z_w_log_var = linear(hid_t_enc, 1, scope='z_w_log_var')
z_w = rect_gaussian_sample(z_w_mean, z_w_log_var)
...
kld = gaussian_kld(z_t_mean, z_t_log_var) + \
        ...
        rect_gaussian_kld(z_w_mean, z_w_log_var)
x_hat[0] = x - tf.clip_by_value(z_w*attunit.write(x_att[0], trans[0],
...
for k in range(1, K):
    ...
    z_w_mean = linear(hid_t_enc, 1, scope='z_w_mean', reuse=True)
    z_w_log_var = linear(hid_t_enc, 1, scope='z_w_log_var', reuse=True)
    z_w = rect_gaussian_sample(z_w_mean, z_w_log_var)
    ...
    kld = kld + gaussian_kld(z_t_mean, z_t_log_var) + \
            gaussian_kld(z_c_mean, z_c_log_var) + \
            rect_gaussian_kld(z_w_mean, z_w_log_var)
    x_hat[k] = x_hat[k-1] - tf.clip_by_value(z_w*attunit.write(x_att[k], trans[k],
        delta_max=delta_max), 0, 1)

hid_c_dec = fc(z_c[0], n_hid, scope='hid_c_dec')
p_att[0] = fc(hid_c_dec, N*N, activation_fn=tf.nn.sigmoid, scope='p_att')
pk = attunit.write(p_att[0], trans[0], delta_max=delta_max)
p = pk
for k in range(1, K):
    ...
    pk = attunit.write(p_att[k], trans[k], delta_max=delta_max)
    p = p + z_w*pk
