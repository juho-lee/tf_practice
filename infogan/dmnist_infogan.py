...
def discriminate(x, reuse=None):
    with tf.variable_scope('Dis', reuse=reuse):
        out = conv(x, 32, 4, stride=2, activation_fn=lrelu)
        out = conv_bn(out, 64, 4, is_training, stride=2, activation_fn=lrelu)
...
def generate(c, z, reuse=None):
    with tf.variable_scope('Gen', reuse=reuse):
        ...
        out = fc_bn(out, height/8*width/8*128, is_training)
        out = tf.reshape(out, [-1, height/8, width/8, 128])
        out = deconv_bn(out, 64, 4, is_training, stride=2)
        out = deconv_bn(out, 32, 4, is_training, stride=2)
...
def test():
    ...
    batch_c = np.repeat(gen_code(10), 10, axis=0)
