...
tf.app.flags.DEFINE_string('save_dir', '../results/mnist/adgm',
...
hid = fc(x, 400)
q_a_mean = linear(hid, 50)
q_a_log_var = linear(hid, 50)
a = gaussian_sample(q_a_mean, q_a_log_var)
hid = tf.nn.relu(linear(a, 400) + linear(x, 400) + linear(y, 400))
q_z_mean = linear(hid, 50)
q_z_log_var = linear(hid, 50)
z = gaussian_sample(q_z_mean, q_z_log_var)

hid = tf.nn.relu(linear(z, 400) + linear(y, 400) + linear(x, 400))
p_a_mean = linear(hid, 50)
p_a_log_var = linear(hid, 50)
hid = tf.nn.relu(linear(z, 400) + linear(y, 400))
...
loss = bernoulli_neg_ll(x, p_x) \
        + gaussian_neg_ll(a, p_a_mean, p_a_log_var) \
        + unit_gaussian_neg_ll(z) \
        - gaussian_neg_ll(a, q_a_mean, q_a_log_var) \
        - gaussian_neg_ll(z, q_z_mean, q_z_log_var)
...
def train():
    ...
    for i in range(FLAGS.n_epochs):
        ...
        for j in range(n_train_batches):
            ...
            _, batch_loss = sess.run([train_op, loss], {x:batch_x, y:batch_y})
        ...
        for j in range(n_valid_batches):
            ...
            batch_loss = sess.run(loss, {x:batch_x, y:batch_y})
...
def test():
    ...
    for i in range(10):
        batch_y[i*10:(i+1):10, i] = 1.
    fig = plt.figure('generated')
    plt.gray()
    plt.axis('off')
    p_gen = sess.run(p_x, {z:batch_z, y:batch_y})
