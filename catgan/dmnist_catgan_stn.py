...
tf.app.flags.DEFINE_string('save_dir', '../results/dmnist/catgan_stn',
...
def discriminate(x, reuse=None):
    with tf.variable_scope('Dis', reuse=reuse):
        ...
        out = conv_bn(out, 64, [3, 3], is_train, activation_fn=lrelu)
        out = pool(out, [3, 3], [2, 2])
        out = conv_bn(out, 128, [3, 3], is_train, activation_fn=lrelu)
        out = conv_bn(out, 10, [1, 1], is_train, activation_fn=lrelu)
        out = fc(flat(out), 10, activation_fn=tf.nn.softmax)
        return out

def generate(z, reuse=None):
    with tf.variable_scope('Gen', reuse=reuse):
        out = fc(z, 4*4*128, activation_fn=lrelu)
        out = tf.reshape(out, [-1, 4, 4, 128])
        out = deconv_bn(out, 96, [3, 3], is_train, stride=[2, 2], activation_fn=lrelu)
        out = deconv_bn(out, 64, [5, 5], is_train, stride=[2, 2], activation_fn=lrelu)
        out = deconv_bn(out, 64, [5, 5], is_train, stride=[2, 2], activation_fn=lrelu)
        out = conv(out, 1, [5, 5], padding='VALID', activation_fn=tf.nn.sigmoid)
        return out

x = tf.placeholder(tf.float32, [None, 56*56])
x_img = tf.reshape(x, [-1, 56, 56, 1])
x_loc = spatial_transformer(x_img, localize(x_img), 28, 28)
y = tf.placeholder(tf.int32, [None])
y_one_hot = one_hot(y, 10) 
z = tf.placeholder(tf.float32, [None, 128])

# discriminator output for real image
p_real = discriminate(x_loc)
