...
tf.app.flags.DEFINE_string('save_dir', '../results/tmnist/cvae_attn_bn',
...
def conv_bn(input, n_ch, scope, padding='SAME'):
    out = conv(input, n_ch, [ksize, ksize], [2, 2],
            padding=padding, activation_fn=None)
    return batch_norm(out, is_training, scope)

x = tf.placeholder(tf.float32, shape=[None, n_in])
x_img = tf.reshape(x, [-1, height, width, 1])
hid_t_enc = conv_bn(x_img, n_ch, 'hid_t_enc1')
hid_t_enc = conv_bn(hid_t_enc, n_ch*2, 'hid_t_enc2')
hid_t_enc = conv_bn(hid_t_enc, n_ch*4, 'hid_t_enc3')
hid_t_enc = conv_bn(hid_t_enc, n_ch*8, 'hid_t_enc4')
...
hid_c_enc = conv_bn(x_att_img, n_ch, 'hid_c_enc1')
hid_c_enc = conv_bn(hid_c_enc, n_ch*2, 'hid_c_enc2', padding='VALID')
hid_c_enc = conv_bn(hid_c_enc, n_ch*4, 'hid_c_enc3', padding='VALID')
...
def deconv_bn(input, n_ch, ksize, stride, scope,
        padding='SAME', activation_fn=tf.nn.relu):
    out = deconv(input, n_ch, [ksize, ksize], [stride, stride],
            padding=padding, activation_fn=None)
...
hid_dec = deconv_bn(hid_dec, n_ch*2, 3, 2, 'hid_dec1', padding='VALID')
hid_dec = deconv_bn(hid_dec, n_ch, 3, 2, 'hid_dec2', padding='VALID')
p_att = flat(deconv_bn(hid_dec, 1, 2, 2, 'p', activation_fn=tf.nn.sigmoid))
    ...
    for i in range(FLAGS.n_epochs):
        ...
        for j in range(n_train_batches):
            ...
            _, batch_neg_ll, batch_kld = \
                    sess.run([train_op, neg_ll, kld],
                            {x:batch_x, is_training:True, learning_rate:lr})
...
def test():
    ...
    batch_x_att, p_recon_att, p_recon = \
            sess.run([tf.clip_by_value(x_att, 0, 1),
                tf.clip_by_value(p_att, 0, 1), p],
                {x:batch_x, is_training:False})
    ...
    p_gen_att, p_gen = sess.run([tf.clip_by_value(p_att, 0, 1), p],
            {z_t:t_noise, z_c:c_noise, is_training:False})
