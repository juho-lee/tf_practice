...
tf.app.flags.DEFINE_string('save_dir', '../results/tmnist/cvae',
...
hid_enc = conv(hid_enc, n_ch*4, [ksize, ksize], [2, 2])
hid_enc = conv(hid_enc, n_ch*8, [ksize, ksize], [2, 2], padding='VALID')
hid_enc = flat(hid_enc)
z_mean = linear(hid_enc, n_lat)
z_log_var = linear(hid_enc, n_lat)
z = gaussian_sample(z_mean, z_log_var)
hid_dec = fc(z, n_ch*8*3*3)
hid_dec = tf.reshape(hid_dec, [-1, 3, 3, n_ch*8])
hid_dec = deconv(hid_dec, n_ch*4, [2, 2], [2, 2])
hid_dec = deconv(hid_dec, n_ch*2, [2, 2], [2, 2])
hid_dec = deconv(hid_dec, n_ch, [2, 2], [2, 2])
p = flat(deconv(hid_dec, 1, [4, 4], [2, 2], padding='VALID',
