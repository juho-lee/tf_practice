...
def to_att(input, **kwargs):
    ...
    W_init = tf.constant_initializer(np.zeros((num_inputs, 5)))
    b_init = tf.constant_initializer(np.zeros(5))

    return tf.contrib.layers.fully_connected(input, 5,
            activation_fn=None,
...
class AttentionUnit(object):
    ...
    def get_att_params(self, att, delta_min=0, delta_max=np.inf):
        ...
        if not np.isinf(delta_max) or delta_min is not 0:
            delta = tf.clip_by_value(delta, delta_min, delta_max)
    ...
    def read(self, x, att, delta_min=0, delta_max=np.inf):
    ...
    def write(self, w, att, delta_min=0, delta_max=np.inf):
        g_x, g_y, sigma, delta, gamma = self.get_att_params(att,
                delta_min=delta_min, delta_max=delta_max)
        F_x, F_y = self.get_filterbanks(g_x, g_y, sigma, delta)
        if type(w) is list:
